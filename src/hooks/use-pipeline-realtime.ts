...
export function usePipelineRealtime() {
  ...
  useEffect(() => {
    ...
    const invalidar = (payload: any) => {
      if (timer) clearTimeout(timer);
      timer = setTimeout(() => {
        const { table, new: newRow, old: oldRow } = payload;
        const targetId = newRow?.cliente_id || oldRow?.cliente_id || newRow?.id || oldRow?.id;

        // Se temos um ID específico, invalidamos apenas esse cliente/proposta
        if (targetId) {
          qc.invalidateQueries({ queryKey: ["cliente", targetId] });
          qc.invalidateQueries({ queryKey: ["cliente-pipeline", targetId] });
        }

        // Invalidações globais com debounce maior
