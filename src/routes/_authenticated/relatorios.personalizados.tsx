...
function Pagina() {
  ...
  const { data: bases } = useQuery({ queryKey: ["report-bases"], queryFn: () => basesFn(), staleTime: Infinity });
  const { data: salvos, isLoading } = useQuery({ queryKey: ["report-saved"], queryFn: () => listaFn(), staleTime: 30_000 });
  ...
  async function salvar() {
    ...
    setBusy(true);
    try {
      await salvarFn({ data: { nome: nome.trim(), report_codigo: base, filtros: { periodo, escopo: "minha" }, visibilidade } });
  ...
  return (
    <div className="mx-auto w-full max-w-5xl space-y-6 p-4 md:p-6">
      <header className="border-b border-border pb-4">
        <p className="text-[11px] font-medium uppercase tracking-[0.16em] text-muted-foreground">Relatórios · Personalizados</p>
        <h1 className="mt-1 text-[26px] font-semibold leading-tight text-foreground">Relatórios personalizados</h1>
        <p className="mt-1 text-sm text-muted-foreground">Monte um relatório a partir de uma base, defina o período e salve para reutilizar.</p>
      </header>

      <Card className="space-y-4 p-4">
        <h2 className="text-[11px] font-medium uppercase tracking-[0.14em] text-muted-foreground">Novo relatório</h2>
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          <div className="space-y-1">
            <label className="text-xs text-muted-foreground">Nome</label>
            <Input value={nome} onChange={(e) => setNome(e.target.value)} placeholder="Ex.: Produção mensal por banco" />
          </div>
          <div className="space-y-1">
          ...
          <div className="space-y-1">
            ...
            <Select value={visibilidade} onValueChange={(v) => setVisibilidade(v as "private" | "shared_team")}>
              <SelectTrigger><SelectValue /></SelectTrigger>
              <SelectContent>
      ...
      <section className="space-y-3">
        <h2 className="text-[11px] font-medium uppercase tracking-[0.14em] text-muted-foreground">Salvos</h2>
        {isLoading ? (
          <div className="space-y-2">{Array.from({ length: 3 }).map((_, i) => <Skeleton key={i} className="h-16" />)}</div>
        ) : !salvos?.length ? (
          <Card className="p-8 text-center text-sm text-muted-foreground">Nenhum relatório personalizado salvo.</Card>
        ) : (
              ...
              <Card key={s.id} className="flex items-center justify-between gap-3 p-3">
                <div className="min-w-0">
                  ...
                  <p className="truncate text-xs text-muted-foreground">
                    Base: {s.report_codigo} · {PERIODO_LABEL[(s.filtros?.periodo as Periodo) ?? "mes"]}
                  </p>
                </div>
