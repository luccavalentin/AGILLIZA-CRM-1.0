...
const grupos: { titulo: string; itens: Item[] }[] = [
  ...
  },
  {
    titulo: "Operação",
    itens: [
      { to: "/relatorios/simulacoes", label: "Simulações", desc: "Volume e conversão", icon: Calculator },
      { to: "/relatorios/propostas", label: "Propostas", desc: "Status e volumes", icon: FileText },
      { to: "/relatorios/operacionais", label: "Operacional", desc: "Execução operacional", icon: Cog },
  ...
  },
  {
    ...
    itens: [
      ...
      { to: "/relatorios/app-cliente", label: "App do Cliente", desc: "Adesão ao aplicativo", icon: Smartphone },
    ],
  },
  {
    titulo: "Financeiro",
  ...
  },
  {
    titulo: "Ferramentas",
    itens: [
      { to: "/relatorios/personalizados", label: "Personalizados", desc: "Monte seus relatórios", icon: Sparkles },
      { to: "/relatorios/exportacoes", label: "Exportações", desc: "Histórico de downloads", icon: Download },
    ],
  },
];
  ...
  return (
    <div className="mx-auto w-full max-w-6xl space-y-6 p-4 md:p-6">
      <header className="border-b border-border pb-4">
        <p className="text-[11px] font-medium uppercase tracking-[0.16em] text-muted-foreground">Central de relatórios</p>
        <h1 className="mt-1 text-[26px] font-semibold leading-tight text-foreground">Relatórios gerenciais</h1>
        <p className="mt-1 text-sm text-muted-foreground">Análises completas em padrão ERP, com filtros, exportação e impressão.</p>
      </header>

      {grupos.map((g) => (
        <section key={g.titulo} className="space-y-3">
          <h2 className="text-[11px] font-medium uppercase tracking-[0.14em] text-muted-foreground">{g.titulo}</h2>
