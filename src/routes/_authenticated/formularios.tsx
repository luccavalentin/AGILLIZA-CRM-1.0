...
const BANCO_LABEL: Record<BancoFormulario, string> = {
  itau: "Itaú",
  bradesco: "Bradesco",
  santander: "Santander",
  inter: "Inter",
...
function Pagina() {
  ...
  const [bancoAtivo, setBancoAtivo] = useState<BancoFormulario>("itau");
  ...
  const porBanco = useMemo(() => {
    const map: Record<BancoFormulario, FormularioBancario[]> = {
      itau: [],
      bradesco: [],
      santander: [],
      inter: [],
    };
    for (const f of data ?? []) map[f.banco]?.push(f);
    return map;
  }, [data]);
  ...
  return (
    <div className="mx-auto w-full max-w-5xl space-y-5 p-4 md:p-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Modelos de formulários em PDF organizados por banco.
            </p>
          </div>
        </div>
      ...
      <Tabs value={bancoAtivo} onValueChange={(v) => setBancoAtivo(v as BancoFormulario)}>
        <TabsList className="grid w-full grid-cols-2 sm:grid-cols-4">
          {BANCOS_FORMULARIO.map((b) => (
        ...
        {BANCOS_FORMULARIO.map((b) => (
          <TabsContent key={b} value={b} className="mt-4 space-y-2">
            {isLoading ? (
              Array.from({ length: 3 }).map((_, i) => <Skeleton key={i} className="h-16 w-full" />)
            ) : porBanco[b].length === 0 ? (
              <Card>
                <CardContent className="py-12 text-center text-sm text-muted-foreground">
                  Nenhum formulário do {BANCO_LABEL[b]} cadastrado ainda.
                </CardContent>
              </Card>
            ) : (
              porBanco[b].map((f) => (
...
function UploadDialog({
  ...
  bancoInicial: BancoFormulario;
  onClose: () => void;
  onDone: () => void;
}) {
  const criar = useServerFn(criarFormulario);
  const [banco, setBanco] = useState<BancoFormulario>(bancoInicial);
      ...
      <DialogContent>
        ...
        <div className="space-y-4">
          <div className="space-y-1.5">
            <Label>Banco</Label>
            <Select value={banco} onValueChange={(v) => setBanco(v as BancoFormulario)}>
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
...
function EditarDialog({
  ...
  const [banco, setBanco] = useState<BancoFormulario>(formulario.banco);
