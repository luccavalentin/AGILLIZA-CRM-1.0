...
export function gerarNomeArquivoPdf(b: any, s: any, d: DetalheBanco | null): string {
  ...
  const docVal = Number(s.valor_despesas_financiadas ?? 0);
...
function drawInfoFinanciamento(
  ...
  const itens: { label: string; valor: string }[] = [
    ...
    { label: "Despesas financiadas", valor: brlOuTraco(s.valor_despesas_financiadas) },
    {
...
export function baixarSimulacaoPDF(input: SimulacaoPdfInput) {
  ...
  const isMista = s.sistema_amortizacao === "B" || (s._multi_prazo ?? false);

  if (!isRapida && (bancos ?? []).length === 1 && !isMista) {
  ...
  const kpis: ReportKpi[] = [
    ...
    { label: "Prazo", valor: s._multi_prazo ? "Múltiplos" : s.prazo ? `${s.prazo} meses` : "—" },
