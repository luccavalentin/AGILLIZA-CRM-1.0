 ...
 */
export function normalizarSistemaAmortizacao(apiValor, requisitado) {
    ...
    if (req === "P" || req.includes("PRICE"))
        return "PRICE";
    if (req === "S" || req.includes("SAC"))
        return "SAC";
    const up = (apiValor ?? "").toUpperCase();
    if (up.includes("PRICE"))
        return "PRICE";
    if (up.includes("SAC"))
        return "SAC";
    if (up === "S")
...
function taxaMensalEquivalente(taxaAnoPct) {
    if (taxaAnoPct == null || !(taxaAnoPct > 0))
        return null;
 ...
 */
export function calcularCET(valorLiberado, parcelas, custosAVista) {
    const bruto = valorLiberado ?? null;
    const fluxo = (parcelas ?? []).map((p) => p.parcela).filter((v) => v > 0);
    if (bruto == null || bruto <= 0 || fluxo.length === 0)
        return null;
    const descontos = Math.max(0, custosAVista ?? 0);
    const principal = bruto - descontos;
    if (!(principal > 0))
        return null;
    const vpl = (i) => fluxo.reduce((acc, parc, idx) => acc + parc / Math.pow(1 + i, idx + 1), -principal);
    let lo = 1e-9;
    let hi = 1;
    if (vpl(lo) < 0 || vpl(hi) > 0)
        return null;
    let mensal = 0;
    for (let k = 0; k < 200; k++) {
        mensal = (lo + hi) / 2;
...
function primeiroVencimentoPadrao() {
    const d = new Date();
 ...
 */
function calcularPlano(principal, n, taxaMesPct, sistema, dataInicial) {
    if (!(principal > 0) || !(n > 0) || !(taxaMesPct > 0))
        return [];
...
export function extrairDetalheBanco(raw) {
    if (!raw || typeof raw !== "object")
        return null;
    const base = raw;
    const r = (base.simulacao ?? base.data ?? base.resultado ?? base);
    const desc = (r.descricaoRespostaBanco ?? {});
    if ((!desc || typeof desc !== "object") && typeof r !== "object")
        return null;
    ...
    const sistema = normalizarSistemaAmortizacao(desc.amortizationType, r.codigoSistemaAmortizacaoBanco ?? r.codigoSistemaAmortizacaoSimulacao);
    ...
    const taxaAno = num(desc.annualRate) ??
    ...
    const primeiraParcelaApi = num(desc.valorPrimeiraPrestacaoComSeguroTac) ??
    ...
    const brutas = Array.isArray(desc.installments) ? desc.installments : [];
    let parcelas = brutas.map(mapParcela).filter((p) => p.parcela > 0);
    let estimadas = false;
    ...
    if (parcelas.length <= 2 && valorFin && prazo && taxaMes) {
        const dataInicial = desc.firstInstallment?.dueDate ??
            brutas[0]?.dueDate ??
    ...
    const tarifaAvaliacaoValor = num(desc.propertyEvaluation) ??
    ...
    const financiamentoBase = num(desc.valorFinanciamento) ??
        num(desc.loanAmount) ??
        num(r.valorFinanciamentoBanco) ??
        num(r.valorFinanciamentoSimulacao);
    const financiamentoTotalRaw = num(desc.valorTotalFinanciamento) ?? num(r.valorTotalFinanciamento) ?? valorFin;
    const despesasApi = num(r.valorDespesasFinanciadas) ??
        num(desc.valorDespesasFinanciadas) ??
        num(desc.expensesFinancedValue);
    const despesasDerivada = financiamentoTotalRaw != null && financiamentoBase != null
    ...
    const despesasFinanciadas = despesasApi != null && despesasApi > 0 ? despesasApi : (despesasDerivada ?? despesasApi);
    ...
    const entradaDerivada = valorImovel != null && financiamentoBase != null
        ? Math.max(0, Math.round((valorImovel - financiamentoBase) * 100) / 100)
        : null;
    const valorEntrada = entradaApi != null && entradaApi > 0 ? entradaApi : (entradaDerivada ?? entradaApi);
    return {
