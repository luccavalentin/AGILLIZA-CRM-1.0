...
const BRADESCO = {
    ...
    "121-L": "Bradesco: a operação não passou nas regras de crédito do banco (limite/valor x renda x prazo). Ajuste valor financiado, entrada ou prazo e reenvie.",
 ...
 */
export function humanizarRespostaErro(json, status, endpoint = "") {
    ...
    if (ctx && (ctx.min != null || ctx.max != null) && ctx.field) {
        ...
        const banco = ctx.bank
            ? String(ctx.bank).replace(/^\w/, (c) => c.toUpperCase())
            : "O banco";
        const ehValor = /amount|value|payment/i.test(String(ctx.field));
        const fmt = ehValor ? moeda : (v) => String(v);
        return `${banco}: ${campo} de ${fmt(ctx.valueProvided)} está fora do intervalo aceito (mínimo ${fmt(ctx.min)} e máximo ${fmt(ctx.max)}). Ajuste o valor e reenvie.`;
    }
    ...
    if (aninhado) {
        try {
            const interno = JSON.parse(aninhado[0]);
...
export function humanizarErroBanco(codigo, mensagem) {
    if (codigo && MAPA[codigo])
        return MAPA[codigo];
    if (codigo && BRADESCO[codigo])
        return BRADESCO[codigo];
