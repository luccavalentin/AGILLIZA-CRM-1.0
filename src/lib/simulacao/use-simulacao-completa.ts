...
export function useSimulacaoCompleta({ duplicar, modoProposta }: OpcoesHook) {
  ...
  const set = (k: string, v: any) => {
    ...
    setF((prev) => {
      ...
      // RENDA INTELIGENTE: Sempre garante o maior valor entre a renda do cliente e a mínima estimada
      if (["valor_financiamento", "valor_imovel", "prazo", "sistema_amortizacao", "renda_total", "renda_conjuge", "renda_price"].includes(k)) {
        // Renda base do cliente (Titular + Cônjuge se compor renda)
        const rendaBaseCliente = Number(next.renda_total || 0) + (next.compoe_renda_conjuge ? Number(next.renda_conjuge || 0) : 0);
        const rendaBasePrice = Number(next.renda_price || 0) + (next.compoe_renda_conjuge ? Number(next.renda_conjuge || 0) : 0);

        if (next.sistema_amortizacao === "P") {
          const evalPrice = avaliarRendaMinima({
            valor_financiamento: next.valor_financiamento,
            valor_imovel: next.valor_imovel,
            prazo_meses: next.prazo || 360,
            taxa_ano: melhorTaxaAno,
            renda_informada: 0,
            sistema: "P",
          });
          if (evalPrice && rendaBasePrice < evalPrice.rendaMinima) {
            next.renda_price = evalPrice.rendaMinima - (next.compoe_renda_conjuge ? Number(next.renda_conjuge || 0) : 0);
          }
        } else if (next.sistema_amortizacao === "S") {
          ...
          if (evalSac && rendaBaseCliente < evalSac.rendaMinima) {
            next.renda_total = evalSac.rendaMinima - (next.compoe_renda_conjuge ? Number(next.renda_conjuge || 0) : 0);
          }
        } else if (next.sistema_amortizacao === "B") {
          const evalSac = avaliarRendaMinima({
            valor_financiamento: next.valor_financiamento,
            valor_imovel: next.valor_imovel,
            prazo_meses: next.prazo || 360,
