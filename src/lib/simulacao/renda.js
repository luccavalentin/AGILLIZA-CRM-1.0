...
import { calcularSimulacao } from "./simulacao-rapida";
/**
 ...
 */
export const COMPROMETIMENTO_BANCOS = {
...
function unwrapApiResponse(raw) {
    if (!raw || typeof raw !== "object")
        return null;
    const r = raw;
    return (r.simulacao ?? r.data ?? r.resultado ?? r);
}
/** Retorna true quando o banco está simulando em tabela PRICE. */
export function isBancoPrice(b) {
    ...
    for (const c of candidatos) {
        ...
        if (s === "P" || s.startsWith("PRICE"))
            return true;
        if (s === "S" || s.startsWith("SAC"))
            return false;
    }
    return false;
}
/** Teto de comprometimento de renda aplicável ao banco conforme mapa de regras. */
export function tetoDoBanco(b) {
 ...
 */
export function rendaMinimaDoBanco(banco) {
    const raw = unwrapApiResponse(banco.raw_response);
 ...
 */
export function rendaMinimaPelosBancos(bancos, rendaInformada) {
    const candidatos = (bancos ?? [])
        ...
        const fonte = b.raw_response?.renda_minima_fonte ?? b.renda_minima_fonte ?? "estimativa";
        if (!parcela || !rendaMinima)
            return null;
        return {
...
export function rendaMinimaParaParcela(primeiraParcela, comprometimentoMax = COMPROMETIMENTO_MAX) {
    if (!Number.isFinite(primeiraParcela) || primeiraParcela <= 0)
        return 0;
...
export function avaliarRendaMinima(params) {
    const { valor_financiamento, prazo_meses, taxa_ano, sistema, renda_informada, valor_imovel } = params;
    // Base do cálculo: valor financiado (correto). Cai para o valor do imóvel
    // apenas quando o financiado não foi informado.
    const base = Number.isFinite(valor_financiamento) && valor_financiamento > 0
        ? valor_financiamento
    ...
    const valorImovelBase = Number.isFinite(valor_imovel) && (valor_imovel ?? 0) > 0 ? valor_imovel : base;
...
export function rendaMinimaSugerida(params) {
    ...
    const sac = avaliarRendaMinima({ ...params, sistema: "S" });
    if (sac)
        fontes.push({ ...sac, detalhe_fonte: "Estimativa local (SAC 30%)" });
    ...
    if (price)
        fontes.push({ ...price, detalhe_fonte: "Estimativa local (PRICE 18%)" });
...
export function calcularMaximoFinanciável(params) {
    ...
    if (parcelaDisponivelParaFinanc <= 0)
        return 0;
 ...
 */
export function limitesLtv(valorImovel, ltvMax) {
    const imovelCentavos = Math.round((Number(valorImovel) || 0) * 100);
    if (imovelCentavos <= 0)
        return { financiamentoMaximo: 0, entradaMinima: 0 };
