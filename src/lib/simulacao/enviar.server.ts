  ...
  try {
    ...
    const proponentes = [
      ...
      ...(sim.data_nascimento_conjuge
        ? [
            {
              nome: sim.nome_conjuge || "Cônjuge",
              vinculo: "cônjuge",
              dataNascimento: sim.data_nascimento_conjuge,
            },
          ]
        : []),
      ...(latestParts || []).map((p) => ({
    ...
    const analise = prazoMaximoParaProponentes(
      proponentes,
      new Date(),
      modoTetoIdade(compoeRendaConjuge),
    );
      ...
      if (recusados.length > 0) {
        ...
        await sbAdminPJ
          ...
          .update({
            ...
            mensagem_banco:
              "Este banco não opera financiamento para pessoa jurídica. Hoje só o Bradesco atende esta modalidade.",
          })
          .in(
            "id",
            recusados.map((b: any) => b.id),
          );
        console.warn(
    ...
    if (bancosParaProcessar.length === 0) {
      ...
      if (ehPJ && bancosEscolhidos.length > 0) {
        const statusPJ = (await recalcularStatusSimulacao(
          simulacaoId,
          supabase,
        )) as EnviarResultado["status"];
    ...
    try {
      const { supabaseAdmin: sbAdminContext } =
      ...
      if (!idOportunidade) {
        ...
        const { data: lockOk, error: lockError } = await sbAdminContext.rpc(
        ...
        console.info(
          `[SIM-PERF] oportunidade_lock_ms=${(performance.now() - tLock).toFixed(0)} result=${lockOk} err=${lockError?.message || "none"}`,
        );

        if (lockError) {
          console.error(
            `[SIM-PERF] Erro crítico na assinatura da RPC eleger_lider_oportunidade:`,
            lockError,
          );
        }
        ...
        if (!lockOk && !lockError) {
          ...
          for (let i = 0; i < 20; i++) {
            const { supabaseAdmin: sbAdmin } =
              await import("@/integrations/supabase/client.server");
            const { data: check } = await sbAdmin
              .from("simulacoes")
        ...
        if (!idOportunidade) {
          ...
          const payloadOp: any = {
            ...
            bancos: bancosParaProcessar.map((b) => ({
              idBanco: b.homefin_id_banco || b.codigo_banco,
          ...
          const resp = await chamarIntegracao<any>("/oportunidade", "POST", payloadOp, {
            simulacao_id: simulacaoId,
          });
          idOportunidade = String(
            resp?.oportunidade?.idOportunidade || resp?.idOportunidade || resp?.id || "",
          );
          ...
          if (idOportunidade) {
            ...
            if (terceiros && terceiros.length > 0) {
              for (const p of terceiros) {
                ...
                try {
                  ...
                  const respPart = await chamarIntegracao<any>(
                    `/oportunidade/${idOportunidade}/participante`,
                    "POST",
                    payloadPart,
                    { simulacao_id: simulacaoId },
                  ...
                  const idPart = String(
                    respPart?.participante?.idParticipante ||
                      respPart?.idParticipante ||
                      respPart?.id ||
                      "",
                  );
                  if (idPart) {
                    const { supabaseAdmin: sbAdminPart } =
                      await import("@/integrations/supabase/client.server");
                    await sbAdminPart
                      .from("simulacao_participantes")
                      .update({ homefin_id_participante: idPart })
                      .eq("id", p.id);
                  }
                } catch (errPart) {
            ...
            const { supabaseAdmin: sbAdminUpdate } =
              await import("@/integrations/supabase/client.server");

            const updatePayload = {
              homefin_id_oportunidade: idOportunidade,
              codigo_oportunidade_homefin:
                resp?.oportunidade?.codigoOportunidade || resp?.codigoOportunidade || null,
            };
    ...
    const processarEmLotes = async (bancos: any[], concurrency: number) => {
      ...
      const worker = async () => {
        while (fila.length > 0) {
          ...
          try {
            const { data: bAtual } = await supabase
              .from("simulacao_bancos")
              .select("status_banco, valor_parcela, homefin_id_simulacao_banco")
              .eq("id", b.id)
              .maybeSingle();
            ...
            console.info(
              `[SIM-PERF] [${b.nome_banco}] oportunidade_start_ms=${(tStartOp - t0).toFixed(0)}`,
            );
            ...
            const { supabaseAdmin: sbAdminErr } =
              await import("@/integrations/supabase/client.server");
            await sbAdminErr
              .from("simulacao_bancos")
              .update({
    ...
    const statusFinal = (await recalcularStatusSimulacao(
      simulacaoId,
      supabase,
    )) as EnviarResultado["status"];
...
async function processarBancoIndividual(
  b: any,
  idOportunidade: string,
  sim: any,
  ...
  try {
    ...
    await sbAdminProc
      ...
      .update({
        ...
        mensagem_banco: "Iniciando preparação...",
    ...
    const idSimulacaoBanco = String(
      respSim?.simulacao?.idSimulacao ?? respSim?.idSimulacao ?? respSim?.id ?? "",
    );
    ...
    await sbAdminProc
      .from("simulacao_bancos")
      .update({
        homefin_id_simulacao_banco: idSimulacaoBanco,
        mensagem_banco: "Simulação bancária preparada.",
      })
      .eq("id", b.id);
    ...
    if (valorDespesasFinanciadas > 0) {
      await sbAdminProc
        .from("simulacao_bancos")
        .update({ mensagem_banco: "Aplicando despesas financiadas..." })
        .eq("id", b.id);
      await chamarIntegracao<any>(
    ...
    await sbAdminProc
      .from("simulacao_bancos")
      .update({ mensagem_banco: "Integrando com a instituição..." })
      .eq("id", b.id);
    const respInt = await chamarIntegracao<any>(
      `/oportunidade/${idOportunidade}/simulacao/${idSimulacaoBanco}/integracao`,
      "POST",
      {},
      { simulacao_id: simulacaoId },
    );
    ...
    const valorParcela = num(
      fonte?.valorParcelaBanco ??
        fonte?.valorParcelaSimulacao ??
        fonte?.simulacao?.valorParcelaBanco ??
        respSim?.valorParcelaBanco,
    );
    ...
    const { avisoDespesasDescartadas } = await import("./aviso-despesas");
    const avisoDespesas = avisoDespesasDescartadas({
      nomeBanco: b.nome_banco,
      totalPedido,
      totalRetornado,
      despesasFinanciadas: valorDespesasFinanciadas,
    });

    if (valorParcela > 0) {
      const taxaJuros =
        fonte?.taxaJurosAnoBanco ??
        respSim?.taxaJurosAnoBanco ??
        fonte?.taxaJurosAno ??
        respSim?.taxaJurosAno;
      const taxaCet =
        fonte?.taxaCetAnoBanco ??
        respSim?.taxaCetAnoBanco ??
        fonte?.taxaCetAno ??
        respSim?.taxaCetAno;
    ...
    const { data: latestSim } = await sbAdminErr
      .from("simulacoes")
      .select("data_nascimento, possui_conjuge, compoe_renda_conjuge, data_nascimento_conjuge")
      .eq("id", simulacaoId)
      .single();
    const { data: latestParts } = await sbAdminErr
      .from("simulacao_participantes")
    ...
    if (latestSim?.possui_conjuge && latestSim?.compoe_renda_conjuge)
      todasDatas.push(latestSim.data_nascimento_conjuge);
    (latestParts || []).forEach((p) => todasDatas.push(p.data_nascimento));

    const idades = todasDatas
      .map((d) => (d ? idadeEmMesesCorridos(d) : 0))
      .filter((i): i is number => !!i && i > 0);
    const idadeMaxMeses = idades.length > 0 ? Math.max(...idades) : 0;

    let prazoMinBanco = null;
    let prazoMaxBanco = null;
    const matchMin =
      msg.match(/superior a[:\s]+(\d+)/i) || msg.match(/mínimo aceito[^:]*[:\s]+(\d+)/i);
    ...
    const statusHttpErro =
      e instanceof IntegracaoBancariaError
        ? ((e as any).statusHttp as number | undefined)
        : undefined;
    const ehTimeoutDeGateway =
      statusHttpErro === 502 || statusHttpErro === 503 || statusHttpErro === 504;
