 ...
 */
const rot = (v) => String(v ?? "").trim();
/** Obrigatórios da Oportunidade + Simulação (bloqueiam o envio). */
export function validarCamposSimulacao(sim) {
    const faltantes = [];
    const add = (campo) => faltantes.push({ campo, onde: "Simulação" });
    if (!sim?.id_operacao_homefin)
        add("Operação");
    if (!rot(sim?.tipo_imovel))
        add("Tipo do imóvel");
    if (!rot(sim?.uso_imovel))
        add("Uso do imóvel (residencial/comercial)");
    if (!rot(sim?.situacao_imovel))
        add("Situação do imóvel (novo/usado)");
    if (!rot(sim?.uf))
        add("UF do imóvel");
    if (!(Number(sim?.valor_imovel) > 0))
        add("Valor do imóvel");
    if (!(Number(sim?.valor_financiamento) > 0))
        add("Valor do financiamento");
    if (!(Number(sim?.prazo) > 0))
        add("Prazo");
    if (!rot(sim?.sistema_amortizacao))
        add("Sistema de amortização (SAC/PRICE)");
    if (!rot(sim?.nome_cliente))
        add("Nome do cliente");
    if (rot(sim?.cpf_cnpj).replace(/\D/g, "").length < 11)
        add("CPF/CNPJ do cliente");
    if (!rot(sim?.data_nascimento))
        add("Data de nascimento");
    if (!rot(sim?.email))
        add("E-mail");
    if (rot(sim?.celular).replace(/\D/g, "").length < 10)
        add("Celular");
    if (!(Number(sim?.renda_total) > 0))
        add("Renda total");
    if (!rot(sim?.estado_civil))
        add("Estado civil");
    return faltantes;
}
/**
 ...
 */
export function validarCamposParticipante(sim, cliente) {
    const faltantes = [];
    const add = (campo) => faltantes.push({ campo, onde: "Cadastro do cliente" });
    const primeiro = (...vs) => vs.map(rot).find((v) => v.length > 0) ?? "";
...
export function mensagemCamposFaltantes(faltantes) {
