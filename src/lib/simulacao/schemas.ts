...
export const completaSchema = z
  ...
  .refine(
    (d) => {
      // Regime de bens: obrigatório quando há casamento/união estável. Solteiro,
      // viúvo e divorciado não têm regime — cobrar deles travaria a simulação.
      // Espelha a regra de `validarCamposSimulacao`.
      if (d.tipo_pessoa === "PJ") return true;
      const ec = String(d.estado_civil ?? "")
        .trim()
        .toLowerCase();
      const casado = ec === "ca" || ec === "ue" || ec === "casado" || ec === "uniao_estavel";
      return !casado || String(d.regime_casamento ?? "").trim() !== "";
    },
    {
      message: "Selecione o regime de casamento",
      path: ["regime_casamento"],
    },
  )
  .refine(
    (d) => {
      // Estado civil do cônjuge: exigido assim que ele é identificado.
      if (d.tipo_pessoa === "PJ") return true;
      const ec = String(d.estado_civil ?? "")
        .trim()
        .toLowerCase();
      ...
      const conjugeIdentificado =
        String(d.nome_conjuge ?? "").trim() !== "" ||
        String(d.cpf_conjuge ?? "").replace(/\D/g, "") !== "";
      if (casado && conjugeIdentificado && String(d.estado_civil_conjuge ?? "").trim() === "") {
        return false;
      }
      return true;
    },
    {
      message: "Selecione o estado civil do cônjuge",
