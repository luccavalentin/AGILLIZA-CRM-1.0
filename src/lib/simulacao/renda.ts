 ...
 */
export const MARGEM_SEGURANCA_RENDA = 0.22;
...
export const COMPROMETIMENTO_BANCOS: Record<string, { SAC: number; PRICE: number }> = {
  default: { SAC: 0.3, PRICE: 0.15 },
  "237": { SAC: 0.3, PRICE: 0.15 }, // Bradesco
  bradesco: { SAC: 0.3, PRICE: 0.15 },
  "033": { SAC: 0.3, PRICE: 0.15 }, // Santander
  santander: { SAC: 0.3, PRICE: 0.15 },
  "341": { SAC: 0.3, PRICE: 0.15 }, // Itaú (Embora não use PRICE em simulações comuns, mantemos o padrão)
  itau: { SAC: 0.3, PRICE: 0.15 },
};
...
export function avaliarRendaMinima(params: {
  ...
  // REGRA ESPECÍFICA SAC: Ajuste de +2.5% adicional sobre o cálculo base.
  // Somando ao ajuste anterior de 4%, temos 6.5% (1.04 * 1.025 = 1.066).
  // Arredondamos para o milhar inferior (truncamento).
  if (sistema === "S") {
    // FATOR SAC: Anterior 1.066 (+2.5% sobre 4%). Novo: +2.5% = 1.066 * 1.025 = 1.09265
    rendaComMargem = rendaComMargem * 1.09265;
    const rendaFinalSac = Math.floor(rendaComMargem / 1000) * 1000;
    return {
      primeiraParcela: prestacaoTotal,
      rendaMinima: rendaFinalSac,
      comprometimento: renda ? prestacaoTotal / renda : null,
      suficiente: renda == null ? null : renda >= rendaFinalSac,
      fonte: "estimativa",
    };
  }

  // REGRA ESPECÍFICA PRICE: Ajuste de +6% adicional sobre o cálculo base.
  // Anterior: 1.1236 (+6% sobre 6%). Novo: +6% = 1.1236 * 1.06 = 1.191016
  let rendaMinima = Math.ceil((rendaComMargem * 1.191016) / 1000) * 1000;
