...
const brlCompacto = (v: number) => {
  ...
  if (Math.abs(n) >= 1_000_000)
    return `R$ ${(n / 1_000_000).toLocaleString("pt-BR", { timeZone: "America/Sao_Paulo",  minimumFractionDigits: 1, maximumFractionDigits: 2 })} mi`;
  if (Math.abs(n) >= 1_000)
    return `R$ ${(n / 1_000).toLocaleString("pt-BR", { timeZone: "America/Sao_Paulo",  maximumFractionDigits: 1 })} mil`;
  return brl(n);
};
const int = (v: number) => (v || 0).toLocaleString("pt-BR", { timeZone: "America/Sao_Paulo" });
const pct = (v: number) =>
  `${Math.min(100, Math.max(0, v || 0)).toLocaleString("pt-BR", { timeZone: "America/Sao_Paulo",  maximumFractionDigits: 1 })}%`;
