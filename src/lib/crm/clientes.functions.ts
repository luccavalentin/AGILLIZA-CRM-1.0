...
const clienteInputSchema = z.object({
  ...
  // Dados de pessoa jurídica exigidos pela integração bancária.
  tipo_empresa: z.string().optional().nullable(),
  faturamento_empresa: z.number().optional().nullable(),
  patrimonio_liquido_empresa: z.number().optional().nullable(),
  capital_social_empresa: z.number().optional().nullable(),
...
export const criarCliente = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<{ id: string }> => {
    ...
    const campos = aplicarPadroesCliente({
      ...
      faturamento_empresa: data.faturamento_empresa ?? null,
      patrimonio_liquido_empresa: data.patrimonio_liquido_empresa ?? null,
      capital_social_empresa: data.capital_social_empresa ?? null,
    ...
    // `campos` já traz os padrões do cadastro. Antes o insert regravava cada
    // coluna a partir de `data`, e cliente novo nascia sem nenhum padrão
    // (nacionalidade, naturalidade, RG, mãe, profissão…) — só o
    // reaproveitamento por documento usava `campos`.
