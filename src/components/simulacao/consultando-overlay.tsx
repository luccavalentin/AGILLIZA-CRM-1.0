...
export function ConsultandoOverlay({
  ...
  const finalizado =
    totalReal > 0 &&
    (listaSimulacoes || []).every((s) => s.estado === "retornada" || s.estado === "erro");
      ...
      <div className="relative w-[min(92vw,416px)] rounded-[22px] border border-[#e8ebf2] bg-white px-8 pb-7 pt-8 text-center shadow-[0_1px_2px_rgba(16,24,40,0.04),0_24px_56px_-20px_rgba(16,24,40,0.18)] animate-in fade-in zoom-in duration-500">
        ...
        <div className="relative flex flex-col items-center">
          ...
          <div
            ...
            style={{
              animation: finalizado ? "none" : "agilliza-batimento 1.6s ease-in-out infinite",
            }}
          >
          ...
          <div className="mt-6 w-full text-center">
            <h2 className="text-[19px] font-semibold tracking-[-0.021em] text-[#000F9F]">
              {titulo}
            </h2>
            <p className="mt-1.5 text-[13.5px] text-slate-500 tabular-nums">
              {totalReal > 0 ? (
                <span>
                  {totalRetornadas} de {totalReal} simulações processadas
                </span>
              ) : (
          ...
          {totalReal > 0 && (
            <div className="mt-7 flex w-full flex-col">
              {Object.entries(bancosAgregados).map(([id, b]: any) => {
                const isUUID =
                  /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(b.nome);
                ...
                return (
                  <div
                    key={id}
                    className="flex items-center gap-3 border-t border-slate-100 px-1 py-3"
                  >
          ...
          {tudoEnviado && !finalizado && (
            <div className="mt-6 w-full rounded-xl border border-slate-200 bg-slate-50/60 px-4 py-3 text-[12.5px] text-slate-500">
              ...
              <Button
                variant="link"
                size="sm"
                className="mt-0.5 h-auto p-0 text-[12.5px] text-[#000F9F]"
                onClick={onTimeout}
              >
                Ver simulações
              </Button>
          ...
          {mostrarErroTimeout && !finalizado && (
            <div className="mt-6 w-full rounded-xl border border-[#f4e6c8] bg-[#fffcf5] px-4 py-4 text-left animate-in fade-in">
              <div className="flex gap-3">
                <AlertCircle
                  className="mt-px h-[18px] w-[18px] shrink-0 text-[#b7791f]"
                  strokeWidth={1.7}
                />
                <div className="flex-1">
                  <p className="text-[13.5px] font-semibold tracking-[-0.01em] text-[#8a5a10]">
                    {bancoLento
                      ? `O ${bancoLento} está demorando mais que o normal`
                      : "A consulta está demorando mais que o normal"}
                  </p>
                  <p className="mt-0.5 text-[12.5px] leading-relaxed text-[#9a7434]">
                    As demais respostas já estão disponíveis. Você pode aguardar ou tentar
                    novamente.
                  </p>
                  <Button
                    variant="outline"
                    size="sm"
                    className="mt-3 h-8 border-[#e8d5a8] text-[12px] text-[#8a5a10] hover:bg-[#fdf6e7]"
                    onClick={() => onReenviar?.()}
                    disabled={!onReenviar}
                  >
                    Tentar reenviar
