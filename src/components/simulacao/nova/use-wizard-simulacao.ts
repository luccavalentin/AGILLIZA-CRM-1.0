...
export function useWizardSimulacao(melhorTaxaAno = 0.1199) {
  ...
  const valido =
    ...
    (!w.prazo_meses_2 || (w.prazo_meses_2 >= PRAZO_MIN && w.prazo_meses_2 <= prazoMaxEfetivo)) &&
    w.tipo_imovel !== "" &&
    w.uso_imovel !== "" &&
    w.situacao_imovel !== "" &&
    w.uf !== "";

  function definirPrazo(valor: number, campo: "prazo_meses" | "prazo_meses_2" = "prazo_meses") {
    if (!Number.isFinite(valor) || valor <= 0) {
      if (campo === "prazo_meses_2") set("prazo_meses_2", null);
      else set("prazo_meses", 0);
      return;
    }

    if (campo === "prazo_meses_2" && valor === w.prazo_meses) {
      toast.error("O segundo prazo não pode ser igual ao primeiro.");
      set("prazo_meses_2", null);
      return;
    }
  ...
  /** Preenche o wizard com dados do CRM */
  function selecionarClienteCRM(c: any) {
    const nascimento = c.data_nascimento ?? "";
    const ec = c.estado_civil?.toLowerCase();
    const sexoCrm = c.sexo?.toUpperCase(); // Supondo que venha do CRM

    setW((prev) => ({
      ...prev,
      nome_cliente: c.nome ?? "",
      cpf_cnpj: c.documento ? c.documento.replace(/\D/g, "") : "",
      email: c.email || "",
      celular: c.telefone_celular || "",
      data_nascimento: nascimento,
      sexo: sexoCrm === "M" || sexoCrm === "F" ? sexoCrm : prev.sexo,
      renda_familiar: c.renda_total_declarada ?? prev.renda_familiar,
      uf: c.imovel_uf || c.endereco_uf || prev.uf,
      // Se tiver cônjuge no CRM, pode vir a ser útil no futuro se o wizard expandir,
      // mas por ora focamos nos campos visíveis do WizardState.
    }));

    if (nascimento) {
      const max = prazoMaximoPorIdade(nascimento);
      if (max && w.prazo_meses > max) {
        set("prazo_meses", max);
      }
    }

    toast.success(`Dados de ${c.nome} importados do CRM.`);
  }
