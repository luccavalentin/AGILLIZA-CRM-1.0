...
export function ContasPage({ tipo }: { tipo: ContaTipo }) {
  ...
  return (
    <div className="mx-auto w-full max-w-6xl space-y-6 p-4 md:p-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            {tipo === "pagar" ? "Fornecedores, parceiros, impostos e despesas." : "Comissões, taxas e outros recebimentos."}
          </p>
        </div>
        ...
        <Table>
          <TableHeader>
            <TableRow className="bg-muted">
              <TableHead className="text-xs uppercase tracking-wide text-muted-foreground">Número</TableHead>
              <TableHead className="text-xs uppercase tracking-wide text-muted-foreground">Descrição</TableHead>
              <TableHead className="text-xs uppercase tracking-wide text-muted-foreground">{tipo === "pagar" ? "Fornecedor" : "Pagador"}</TableHead>
              <TableHead className="text-xs uppercase tracking-wide text-muted-foreground">Categoria</TableHead>
              <TableHead className="text-xs uppercase tracking-wide text-muted-foreground">Vencimento</TableHead>
          ...
          <TableBody>
            ...
            {data?.itens.map((c) => (
              <TableRow key={c.id} className="even:bg-muted/40 dark:even:bg-muted/60">
                <TableCell className="cursor-pointer font-medium" onClick={() => setDetalheId(c.id)}>{c.numero}</TableCell>
                <TableCell className="cursor-pointer" onClick={() => setDetalheId(c.id)}>{c.descricao}</TableCell>
                ...
                <TableCell><ContaStatusBadge status={c.status_efetivo} /></TableCell>
                <TableCell>
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      <DropdownMenuItem onClick={() => setDetalheId(c.id)}>Ver detalhes</DropdownMenuItem>
                      {c.status !== "paga" && c.status !== "cancelada" && c.status !== "estornada" && (
                        <DropdownMenuItem onClick={() => setBaixarConta(c)}>
                          {tipo === "pagar" ? "Baixar" : "Confirmar recebimento"}
                        </DropdownMenuItem>
                      )}
                      {c.status !== "estornada" && c.valor_pago > 0 && (
                        <DropdownMenuItem className="text-destructive" onClick={() => setEstorno({ id: c.id, acao: "estornar" })}>
                          Estornar
                        </DropdownMenuItem>
                      )}
                      {c.status !== "cancelada" && c.status !== "estornada" && (
                        <DropdownMenuItem className="text-destructive" onClick={() => setEstorno({ id: c.id, acao: "cancelar" })}>
                          Cancelar
                        </DropdownMenuItem>
                      )}
      ...
      <BaixarDialog tipo={tipo} conta={baixarConta} open={!!baixarConta} onOpenChange={(o) => !o && setBaixarConta(null)} />
      <EstornarDialog
        tipo={tipo}
