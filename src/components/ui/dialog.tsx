...
>(({ className, children, ...props }, ref) => {
  // Quem passa `p-0`/`px-0` cuida do próprio espaçamento (visualizadores,
  // relatórios em tela cheia). Os demais ganham a margem lateral aqui, em vez
  // de cada modal lembrar de pôr a sua — era por isso que o conteúdo encostava
  // na borda.
  const semPadding = /(^|\s)(p|px)-0(\s|$)/.test(className ?? "");
  return (
    <DialogPortal>
      <DialogOverlay />
      ...
      */}
        <div
          className={cn(
            "brand-scroll relative z-1 flex min-h-0 min-w-0 flex-1 flex-col overflow-y-auto overflow-x-hidden",
            !semPadding && "px-5 sm:px-6",
          )}
        >
          {children}
        </div>
...
const DialogHeader = ({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
  <div
    className={cn(
      // O espaço lateral vem do corpo do modal; aqui só o de cima e de baixo.
      // `pr-8` guarda o lugar do botão de fechar, que fica sobre o cabeçalho.
      "flex shrink-0 flex-col space-y-1.5 pb-4 pr-8 pt-6 text-center sm:text-left",
      className,
    )}
    {...props}
  />
