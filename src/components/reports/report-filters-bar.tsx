...
export function VisionSelector({ escopo, onChange, podeEquipe, podeGeral }: { escopo: Escopo; onChange: (e: Escopo) => void; podeEquipe: boolean; podeGeral: boolean }) {
  const opts: Escopo[] = ["minha", ...(podeEquipe ? (["equipe"] as Escopo[]) : []), ...(podeGeral ? (["geral"] as Escopo[]) : [])];
  return (
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {!!bancos?.length && (
          <Select value={filtros.banco ?? "__all"} onValueChange={(v) => set({ banco: v === "__all" ? undefined : v })}>
            <SelectTrigger className="h-9 w-44"><SelectValue placeholder="Banco" /></SelectTrigger>
            <SelectContent>
        ...
        <Input value={filtros.busca ?? ""} onChange={(e) => set({ busca: e.target.value || undefined })} placeholder="Buscar…" className="h-9 w-48" />

        {(chips.length > 0 || temExtra) && (
          <Button variant="ghost" size="sm" onClick={() => onChange({ periodo: filtros.periodo, escopo: filtros.escopo })}>Limpar</Button>
        )}
      </div>
        ...
        <div className="flex flex-wrap gap-1.5">
          {chips.map((c) => (
            <Badge key={c.key} variant="secondary" className="gap-1">
              {c.label}
              <button type="button" onClick={() => set({ [c.key]: undefined } as Partial<ReportFiltros>)} aria-label="Remover filtro">
                <X className="h-3 w-3" />
              </button>
            </Badge>
          ))}
