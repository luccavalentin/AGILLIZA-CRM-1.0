    ...
    <div className="mx-auto w-full max-w-[1600px] space-y-6 p-3 sm:p-4 md:space-y-8 md:p-6">
      ...
      {error ? (
        ...
        <>
          ...
          {(data?.minis?.length ?? 0) > 0 && (
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
              {data.minis.map((m: any) => (
                <MiniMetric
                  key={m.label}
                  label={m.label}
                  valor={m.valor}
          ...
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-12">
            {data?.evolucao && (data.evolucao.dados?.length ?? 0) > 1 && (
            ...
            {data?.funil && (data.funil.etapas?.some((e: any) => e.valor > 0) ?? false) && (
            ...
            <div className="lg:col-span-4">
              <PanelCard
                titulo={data?.chart?.titulo ?? "Resumo"}
                subtitulo={data?.chart?.subtitulo}
                abrirTo={abrirTo}
                onOpen={data?.chart?.titulo ? () => abrirDetalhe(data.chart.titulo) : undefined}
              >
                {data?.chart?.porBanco ? (
                  <MetricList
                    items={data.chart.dados.map((d: any) => ({ label: d.label, valor: d.valor }))}
                    colorByBank
                    onItemClick={(label, valor) =>
                      abrirDetalhe(label, valor.toLocaleString("pt-BR"))
                    }
          ...
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
            <div className="lg:col-span-1">
              <PanelCard
                titulo={data?.ranking?.titulo ?? "Ranking"}
                onOpen={data?.ranking?.titulo ? () => abrirDetalhe(data.ranking.titulo) : undefined}
              >
                <MetricList
            ...
            <div className="lg:col-span-2">
              ...
              >
                {!data?.alertas || (data.alertas?.length ?? 0) === 0 ? (
          ...
          {data?.distribuicao && (data.distribuicao.dados?.length ?? 0) > 0 && (
            <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
              ...
              {data?.recusadasPorBanco && (data.recusadasPorBanco.itens?.length ?? 0) > 0 && (
                <PanelCard
                ...
                >
                  <MetricList
                    ...
                    colorByBank
                    onItemClick={(label, valor) =>
                      abrirDetalhe(label, valor?.toLocaleString("pt-BR") ?? "0")
                    }
                  />
                </PanelCard>
              )}
          ...
          {(data.porTipoSimulacao || data.clientesPorEtapa) && (
            <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
              {data.porTipoSimulacao && (data.porTipoSimulacao.dados?.length ?? 0) > 0 && (
                <PanelCard
              ...
              {data.clientesPorEtapa && (data.clientesPorEtapa.dados?.length ?? 0) > 0 && (
                <PanelCard
                ...
                >
                  <div
                    ...
                    style={{
                      height: Math.min(
                        360,
                        Math.max(180, (data.clientesPorEtapa.dados?.length ?? 0) * 40 + 44),
                      ),
                    }}
                  >
                    <ReportChartView
          ...
          {(data.volumePorBanco || data.topOperadores || data.financeiroResumo) && (
            <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
              {data.volumePorBanco && (data.volumePorBanco.dados?.length ?? 0) > 0 && (
                <PanelCard
                ...
                >
                  <MetricList
                    items={(data.volumePorBanco.dados ?? []).map((d: any) => ({
                      label: d.label,
                      valor: d.valor,
                      display: (d.valor ?? 0).toLocaleString("pt-BR", {
                        style: "currency",
                        currency: "BRL",
              ...
              {data.topOperadores && (data.topOperadores.dados?.length ?? 0) > 0 && (
                <PanelCard
              ...
              {data.financeiroResumo && (
                ...
                >
                  <div className="space-y-2">
                    {(data.financeiroResumo.itens ?? []).map((i: any) => (
                      <button
                        type="button"
                        key={i.label}
