...
export function ReportChartView({ chart }: { chart: ReportChart }) {
  const fmt = chart.moeda ? (v: number) => formatBRL(Number(v)) : (v: number) => Number(v).toLocaleString("pt-BR");
    ...
    return (
      <ResponsiveContainer width="100%" height="100%">
        <LineChart data={chart.dados}>
          ...
          <Line type="monotone" dataKey="valor" name={chart.serie1 ?? "Total"} stroke="var(--chart-1)" strokeWidth={2} dot={false} />
          {chart.serie2 && <Line type="monotone" dataKey="valor2" name={chart.serie2} stroke="var(--chart-3)" strokeWidth={2} dot={false} />}
        </LineChart>
      </ResponsiveContainer>
    );
  }
  ...
  return (
    <ResponsiveContainer width="100%" height="100%">
      <BarChart data={chart.dados}>
        <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" />
        <XAxis dataKey="label" tick={{ fontSize: 11 }} stroke="hsl(var(--muted-foreground))" />
        <YAxis tick={{ fontSize: 11 }} stroke="hsl(var(--muted-foreground))" width={56} tickFormatter={fmt} />
        <Tooltip contentStyle={tooltipStyle} formatter={(v: number) => fmt(v)} />
