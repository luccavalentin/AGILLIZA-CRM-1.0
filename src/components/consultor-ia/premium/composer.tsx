...
export function ConsultorComposer({
  pergunta,
  setPergunta,
  onEnviar,
  streaming,
}: ConsultorComposerProps) {
    ...
    <div className="mx-auto w-full max-w-3xl px-4 pb-6">
      <div className="relative group rounded-[1.5rem] border border-border bg-card p-1 shadow-xl shadow-black/5 transition-all focus-within:border-primary/50 focus-within:ring-2 focus-within:ring-primary/20">
        ...
        <div className="flex items-center justify-between px-2 pb-1">
          <div className="flex gap-1">{/* Espaço para futuras ações de anexo */}</div>
          <Button
            size="icon"
            onClick={onEnviar}
      ...
      <p className="mt-3 text-center text-[10px] text-muted-foreground/60">
        O Consultor IA pode apresentar informações imprecisas. Verifique sempre os dados
        importantes.
      </p>
    </div>
  );
...
export const SUGESTOES = [
  ...
  {
    icone: Zap,
    titulo: "FGTS habitacional",
    prompt: "Como funciona o uso de FGTS no financiamento?",
  },
  {
    icone: BookMarked,
    titulo: "Documentação",
    prompt: "Quais documentos são obrigatórios na proposta?",
  },
