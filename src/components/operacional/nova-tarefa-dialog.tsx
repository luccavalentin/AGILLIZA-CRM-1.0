...
export function NovaTarefaDialog({ onCriada }: { onCriada: () => void }) {
  ...
  async function salvar() {
    ...
    try {
      ...
      setAberto(false);
      setTitulo(""); setDescricao(""); setPrazo(""); setResponsavel(""); setCliente(""); setChecklist([]);
      onCriada();
    } catch (e) {
    ...
    <Dialog open={aberto} onOpenChange={setAberto}>
      ...
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-lg">
              ...
              <ul className="mt-2 space-y-1">
                {checklist.map((it, i) => (
                  <li key={i} className="flex items-center justify-between rounded-md bg-muted px-2 py-1 text-sm">
                    <span>{it}</span>
                    <button type="button" onClick={() => setChecklist((l) => l.filter((_, j) => j !== i))}>
        ...
        <DialogFooter>
          ...
          <Button onClick={salvar} disabled={salvando}>Criar tarefa</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
