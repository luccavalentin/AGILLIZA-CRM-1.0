-- Aprovação condicionada move a esteira do cliente no CRM.
--
-- `proposta_sincronizar_esteira` mapeava `credito_aprovado` para a etapa
-- "credito_aprovado" do CRM, mas `credito_condicionado` caía no ELSE NULL: a
-- proposta aprovada com condições não andava a esteira do cliente e a
-- notificação saía como "Proposta atualizada". Só essas duas linhas mudam; o
-- resto da função é o mesmo em produção em 17/09/2026.
...
BEGIN
  ...
  v_stage := CASE
    ...
    WHEN NEW.status IN ('credito_aprovado','credito_condicionado') THEN 'credito_aprovado'
