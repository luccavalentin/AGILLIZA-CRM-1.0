...
BEGIN
  ...
  -- Evolução: o primeiro ponto exibido é sempre 0%.
  -- Os demais pontos acumulam a última movimentação real dentro da janela,
  -- mantendo uma curva ascendente conforme o processo avança no período.
  SELECT jsonb_agg(jsonb_build_object(
    'dia', d.dia,
    'percentual', CASE
      WHEN d.idx = 0 THEN 0
      ELSE LEAST(100, GREATEST(0, ROUND(
        COALESCE((
          ...
          WHERE h.cliente_id = _cid
            AND h.created_at >= (v_inicio_janela + INTERVAL '1 day')
