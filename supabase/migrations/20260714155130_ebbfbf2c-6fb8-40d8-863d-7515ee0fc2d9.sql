...
BEGIN
  ...
  LEFT JOIN public.profiles pf ON pf.id = COALESCE(pr.responsavel_id, v_resp)
  ...
  SELECT jsonb_agg(jsonb_build_object(
    ...
    'percentual', ROUND(
      COALESCE((
        SELECT max(ps.ordem)::numeric FROM public.cliente_pipeline_historico h
        JOIN public.pipeline_stages ps ON ps.id = h.stage_id
        WHERE h.cliente_id = _cid AND h.created_at < (d.dia + INTERVAL '1 day')
      ), 0) / NULLIF(v_total, 0) * 100
    )
  ) ORDER BY d.dia)
  INTO v_evolucao
  FROM (
    SELECT (CURRENT_DATE - offs)::date AS dia FROM generate_series(6, 0, -1) AS offs
  ) d;
...
GRANT EXECUTE ON FUNCTION public.portal_acompanhamento(uuid) TO anon, authenticated;
