-- Limpa o registro de teste do fechamento automático e junta os alertas ao
-- painel de saúde: o que está aberto agora e o que abriu e fechou nas
-- últimas 24 h (é isso que conta a história que ninguém via — a proposta que
-- ficou presa às 3 da manhã e destravou às 7).

delete from public.sistema_alertas where titulo = 'teste de fechamento automático';
...
begin
  ...
  select jsonb_build_object(
    'agendadores', (
      select coalesce(jsonb_agg(jsonb_build_object(
        'nome', case j.jobid when 2 then 'sincronizar propostas'
                             ...
                             else coalesce(j.jobname, 'job ' || j.jobid) end,
        'ultima_em', u.ultima,
    ...
    'alertas_abertos', (
      select coalesce(jsonb_agg(jsonb_build_object(
        'tipo', tipo, 'titulo', titulo, 'valor', valor, 'desde', detectado_em
      ) order by detectado_em), '[]'::jsonb)
      from public.sistema_alertas where resolvido_em is null
    ),
    'alertas_resolvidos_24h', (
      select count(*) from public.sistema_alertas
      where resolvido_em > now() - interval '24 hours'
    ),
    'http_falhas_1h', (
