-- Esteira do CRM: desempate estável pelo id entre clientes com o mesmo nome
-- (5 nomes repetidos em 18/09/2026). Antes a ordem entre homônimos era
-- arbitrária, tanto no código antigo quanto em 20260919021000.
...
as $$
  select coalesce(jsonb_agg(linha order by nome, id), '[]'::jsonb)
  from (
    select
      c.nome,
      c.id,
