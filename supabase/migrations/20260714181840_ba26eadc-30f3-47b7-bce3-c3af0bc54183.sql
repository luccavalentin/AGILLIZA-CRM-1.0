...
BEGIN
  ...
  -- Evolução: começa em 0 no início da janela (7 dias atrás) e sobe
  -- conforme cada movimentação real do funil dentro da janela.
  -- Movimentações anteriores à janela NÃO servem de base.
  SELECT jsonb_agg(jsonb_build_object(
    'dia', d.dia,
    'percentual', LEAST(100, GREATEST(0, ROUND(
      COALESCE((
