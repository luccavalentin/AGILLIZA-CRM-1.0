...
1. **`listarDemandas` — bug do `.or()` com `in.(...)`**: no escopo *Minhas*, os filtros `id.in.(uuid1,uuid2)` e `cliente_id.in.(uuid1,uuid2)` eram concatenados dentro de `.or(...)`; as vírgulas do `in.(...)` colidem com o separador de OR do PostgREST e derrubavam a consulta quando o usuário tinha ≥2 demandas onde é apenas participante ou ≥2 clientes-parceiros. Reescrito para emitir uma condição `eq.` por id.
...
- **Recebimento**: aparece no feed do responsável (`demanda.criada`) e na lista *Minhas*.
...
### Recebimento
- [ ] Responsável e participantes vêem a demanda em *Minhas* (escopo já cobre criador/responsável/participante/parceiro do cliente).
